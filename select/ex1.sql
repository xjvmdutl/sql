...
select sum(case when gender = 'm' then 1 else 0 end) as 남직원,
	   sum(case when gender = 'f' then 1 else 0 end) as 여직원
  from employees;
  
select *
from (select count(*) as 여직원
	  from employees
      where gender = 'f') ,
      (select count(*) as 남직원
	  from employees
...
-- 문제5.
-- 부서는 총 몇 개가 있나요?
select count(distinct(dept_name)) as 부서종류 
from departments;
-- 문제6.
-- 현재 부서 매니저는 몇 명이나 있나요?(역임 매너저는 제외)
select count(*) as 'manager 수'
from dept_manager
where to_date like '9999-01-01';
-- 문제7.
-- 전체 부서를 출력하려고 합니다. 순서는 부서이름이 긴 순서대로 출력해 보세요.
select distinct(dept_name) as '부서 종류'
from departments
